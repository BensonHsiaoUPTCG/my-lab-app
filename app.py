...
st.markdown("""
    ...
    /* Force text color to be dark even in Dark Mode */
...
FILE_PATH = 'inventory_v2.csv'
HISTORY_PATH = 'history_log.csv'
USER_DB_PATH = 'users.csv' # New User Database
IMAGE_DIR = 'images'
...
if 'role' not in st.session_state:
    st.session_state['role'] = 'Student' # Default View Only

st.sidebar.title("🔬 IMS Mobile (v2.1)")

# --- Authentication Widget (Updated) ---
if not st.session_state['logged_in']:
    ...
    with tab_signup:
        ...
        # IMPROVED: Role Selection Logic with Admin Key
        ...
        if st.button("Create Account"):
            if new_user and new_pass:
                ...
                if "Admin" in role_mode:
                    if admin_key == "1234": # Simple Hardcoded Key for Demo
                        role_to_save = "Admin"
                    else:
                        st.error("❌ Invalid Admin Key! Cannot create Admin account.")
                        valid = False
